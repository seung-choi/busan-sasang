...
export const SearchList = ({ deviceData, featureData, onFloorChange }: SearchListProps) => {
  ...
  const handleDeviceEdit = (e: React.MouseEvent, device: Device) => {
    e.stopPropagation();
    navigate(`/admin/dashboard/device/management?edit=true&deviceId=${device.id}`);
  };
  ...
  return (
    <div className="flex flex-col h-full shadow-lg overflow-hidden">
      <div className="p-4 border-b border-gray-200">
        ...
        <div
          className={`flex items-center justify-between mt-3 px-3 py-2 rounded-lg cursor-pointer transition-all ${
            showUnassigned ? 'bg-primary-100 text-primary-700' : 'bg-gray-50 text-gray-700 hover:bg-gray-100'
          }`}
        ...
        >
          ...
          <span className={`text-xs font-medium px-2 py-1 rounded-full ${
            showUnassigned ? 'bg-primary-200 text-primary-800' : 'bg-gray-200 text-gray-700'
          }`}>
            {unassignedCount}
          </span>
        </div>
          ...
          <div className="flex space-x-2">
            ...
            >
              <div className="relative">
                <Select.Trigger className="h-8 px-2 text-xs overflow-hidden p-1" placeholder='층 선택' />
                <div className="absolute right-2 top-1/2 transform -translate-y-1/2 pointer-events-none w-3 h-3">
                  <AccordionIcon />
                </div>
              </div>
              <Select.Content>
                <Select.Item value="" className="text-xs">전체 층</Select.Item>
            ...
            >
              <div className="relative">
                <Select.Trigger className="h-8 px-2 text-xs overflow-hidden p-1" placeholder="카테고리"/>
                <div className="absolute right-2 top-1/2 transform -translate-y-1/2 pointer-events-none w-3 h-3">
                  <AccordionIcon />
                </div>
              </div>
              <Select.Content>
                <Select.Item value="" className="text-xs">전체 카테고리</Select.Item>
